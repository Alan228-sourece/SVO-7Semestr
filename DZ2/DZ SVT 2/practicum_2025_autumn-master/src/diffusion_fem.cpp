...
double Problem::basis_func(const Cell &c, const Node &n, double x_, double y_)
{
	...
	//Здесь всё делаем по формулам 15-17 для базисных ф-ий узлов тереугольников
	else if(n_ind == 1){
...
rMatrix Problem::basis_func_grad(const Cell &c, const Node &n)
{
	...
	//считаем градиенты базисных функций опять же по формулам 15-17  
	else if(n_ind == 1){
		grad(0,0) = (y[0] - y[2]);
    	grad(1,0) = - (x[0] - x[2]);
...
void Problem::assembleLocalSystem(const Cell &c, rMatrix &A_loc, rMatrix &rhs_loc)
{
	 ...
	 A_loc = rMatrix(3,3);
	 rhs_loc = rMatrix(3,1);
	//заполняем матрицу грамма ф-ий фи	
	 for(unsigned i = 0; i < 3; i++){
         // fill matrix and rhs
		for (unsigned j = 0; j <= i; j++) {
			A_loc(i, j) = (basis_func_grad(c, nodes[j]).Transpose() * D * basis_func_grad(c, nodes[i]))(0, 0);
			A_loc(j, i) = A_loc(i, j);
		}
		// пр часть по методу средних 
